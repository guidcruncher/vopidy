      ...
      <v-tabs-window v-model="tab">
        <v-tabs-window-item value="system">
          <div class="pa-2">
            <h3>System</h3>
            <v-switch
              v-model="settings.enableEqualiser"
              label="Enable 10-band Equaliser"
            ></v-switch>
            <v-switch
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                    ...
                    ></v-select>
                    <v-text-field
                      label="Read chunk size (ms)"
                      v-model="settings.snapcastChunkMs"
                    ></v-text-field>
                    <v-text-field
                      label="End-to-end latency (ms)"
                      v-model="settings.snapcastBuffer"
                    ></v-text-field>
                    <span>
                      Please see
  ...
  methods: {
    ...
    saveSettings() {
      vopidy('core.config-get', {}).then((res) => {
        ...
        this.settings.clockType = this.clockType ?? this.settings.clockType
        vopidy('core.config-set', { config: this.settings }).then((res) => {
